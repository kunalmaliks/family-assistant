...
- Switchable AI provider (lib/ai-provider.ts) — `detectCalendarEvents`, `checkCalendarDuplicate`, `generateDailyBrief`, attachment OCR (`extractDocumentText` in lib/gmail.ts), and chat (`app/api/chat/route.ts`, via `generateChatResponse`) all route through this module, which branches on a single `AI_PROVIDER` env var between Claude (default — Sonnet 4.6 for chat, Haiku for the other 4) and Gemini (`gemini-3.5-flash-lite` for all 5, including chat — one switch, no separate chat-only toggle, confirmed with user despite chat being the higher quality-risk surface). Added because the 4 background sites run on every synced email/attachment and were driving Claude API cost; extended to chat 2026-09-23 at user request. Gemini's tool-calling (`config.tools.functionDeclarations` + `parametersJsonSchema`, reusing the same JSON Schema already used for Claude's `input_schema` — no translation needed) and multi-turn history (`role: "model"` vs Claude's `"assistant"`) are both live-tested and working, including a real `suggest_calendar_event` tool-call round-trip. Originally pinned to `gemini-2.5-flash-lite` per user preference over the auto-updating `-latest` alias, but that model returned a hard 404 ("no longer available to new users") on live testing — switched to `gemini-3.5-flash-lite` (Google's own error message's recommended replacement).
