...
import { generateVisionText } from "./ai-provider"
...
export async function syncEmailsForUser(
  ...
  for (const messageId of allMessageIds) {
    ...
    try {
      ...
      if (userId && insertedEmail) {
        const combinedText = `${body}${attachmentText ? "\n" + attachmentText : ""}`
        const receivedIso = new Date(dateStr).toISOString()
        await processEmailForCalendar(subject, combinedText, receivedIso, insertedEmail.id, userId, accessToken, refreshToken, timezone)
...
async function extractDocumentText(
  ...
  try {
    const text = await generateVisionText(
      buffer,
      mediaType,
      "Extract all text from this document. Return only the extracted text with no commentary, labels, or explanation.",
      1500
    )
    ...
    console.error(`[gmail] OCR failed (${filename}):`, e)
    return null
  }
}
...
async function extractAttachmentText(gmail: any, messageId: string, attachmentId: string, mimeType: string, filename: string): Promise<string | null> {
  ...
  try {
    ...
    if (isPdf) {
      ...
      return extractDocumentText(buffer, "application/pdf", filename)
    }

    if (isImage) {
      const resolvedMime = IMAGE_MIME_TYPES.has(mimeType) ? mimeType : `image/${ext === "jpg" ? "jpeg" : ext}`
      return extractDocumentText(buffer, resolvedMime, filename)
    }
